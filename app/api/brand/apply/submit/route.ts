...
//
// Finalizes a BrandApplication draft into 'pending' status. Validates all
// required fields, checks brand name collision (allowing the user's own draft
// to "claim" the name), sends the "we received your application" email.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Rate-limit by IP to prevent rapid resubmission attempts
    ...
    const { brandName, website, description, shopifyConfirmed } = body;
    ...
    // Block if name is taken by an active Client OR a pending/approved
    // application from someone else OR a draft from someone else within
    // the last 7 days.
