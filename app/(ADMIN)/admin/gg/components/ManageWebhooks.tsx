...
export default function ManageWebhooks() {
  ...
  // New state for handling UI messages
  ...
  const fetchWebhooks = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setWebhooks([]); // Clear table on error
  ...
  const registerStandard = async () => {
    ...
    try {
        ...
        setSuccessMessage("Webhook successfully registered!");
        fetchWebhooks();
    } catch (e: any) {
        setErrorMessage(e.message);
        setLoading(false);
        ...
        <Table.Root>
          ...
          <Table.Body>
            ...
            {webhooks.length === 0 && !loading && !errorMessage && (
                <Table.Row><Table.Cell colSpan={5}>No webhooks found. Enter a Client ID and fetch.</Table.Cell></Table.Row>
            )}
          </Table.Body>
        </Table.Root>
      </Card>
